...
function setBrushUp(featureAxis, brushOverlay, tooltipValues) {
  featureAxis.each(function (d) {
    ...
    g.append("use")
      ...
      .style(
        "cursor",
        `url('data:image/svg+xml,${setSize(encodeURIComponent(getArrowTopCursor()), 14)}') 7 7, auto`,
      );
    const hit = g
      .append("rect")
...
function setBrushDown(featureAxis, brushOverlay, tooltipValues) {
  featureAxis.each(function (d) {
    ...
    g.append("use")
      ...
      .style(
        "cursor",
        `url('data:image/svg+xml,${setSize(encodeURIComponent(getArrowBottomCursor()), 14)}') 7 7, auto`,
      );
    const hit = g
      .append("rect")
      .attr("class", "spcd3-handle-hitbox")
...
function brushDown(cleanDimensionName, event, d, tooltipValues, window) {
  ...
  if (yPosTop == TOP_AXIS_LOW_VALUE && yPosBottom == BOTTOM_AXIS_VALUE) {
    ...
    select("#rect_" + cleanDimensionName).style(
      "cursor",
      `url('data:image/svg+xml,${setSize(encodeURIComponent(getArrowTopAndBottom()), 20)}') 10 10, auto`,
    );
  }
...
function setCursorForDimensions(d, featureAxis) {
  if (getDimensionPosition(d.name) == 0) {
    featureAxis
      ...
      .style(
        "cursor",
        `url('data:image/svg+xml,${setSize(encodeURIComponent(getArrowRight()), 12)}') 12 6, auto`,
      );
    ...
    featureAxis
      ...
      .style(
        "cursor",
        `url('data:image/svg+xml,${setSize(encodeURIComponent(getArrowLeft()), 12)}') 0 6, auto`,
      );
  } else {
    featureAxis
      .select(".dimension")
      .style(
        "cursor",
        `url('data:image/svg+xml,${setSize(encodeURIComponent(getArrowLeftAndRight()), 12)}') 6 6, auto`,
      );
  }
}
  ...
  invertIcon
    ...
    .each(function (d) {
      ...
      select(this)
        ...
        .style(
          "cursor",
          `url('data:image/svg+xml,${setSize(encodeURIComponent(getArrowDownCursor()), 12)}') 6 6, auto`,
        );
    });
  invertIcon
    .append("use")
