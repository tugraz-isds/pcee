...
function getArrowDownCursorMeta() {
  return {
    ...
    viewBoxMinX: 0,
    viewBoxMinY: 0,
    viewBoxWidth: 3.9687499,
    viewBoxHeight: 3.96875,
  };
}
...
function transformer() {
  ...
  scale.clamp = function (_) {
    return arguments.length
      ? ((clamp = _ ? true : identity$2), rescale())
      : clamp !== identity$2;
  };
...
function formatLocale(locale) {
  ...
  function newFormat(specifier, options) {
    ...
    var prefix =
        (options && options.prefix !== undefined ? options.prefix : "") +
        (symbol === "$"
          ? currencyPrefix
      ...
      suffix =
        ...
        (options && options.suffix !== undefined ? options.suffix : "");
        ...
        valuePrefix =
          (valueNegative
            ...
            : sign === "-" || sign === "("
              ...
              : sign) + valuePrefix;
        valueSuffix =
          (type === "s" && !isNaN(value) && prefixExponent !== undefined
...
function getLongestTickLabel(data, labelKey) {
  const uniqueLabels = Array.from(
    new Set(data.map((d) => String(d[labelKey] ?? ""))),
  ).filter((s) => s.length > 0);
  const ticks =
    ...
    tips.join(
      (enter) =>
        enter
          ...
          .attr(
            "id",
            `tooltip-record-select-${cleanString(String(records[hoverlabel] ?? ""))}`,
          )
...
function interpolatePathCommands(
  ...
  var _ref =
      _typeof(interpolateOptions) === "object"
        ? interpolateOptions
        : {
...
function invert(dimension) {
  ...
  selectedRecords.forEach((record) => {
    const path = parcoords.newDataset.find(
      (d) => cleanString(d[hoverlabel]) === record,
    );
    if (!isRecordInactive(record)) {
      createToolTipForValues(path, true);
    }
...
function getRecordWithId(recordId) {
  const item = parcoords.currentPosOfDims.find(
    (object) => object.recordId == recordId,
  );
  return item.key;
}
...
function setBrushUp(featureAxis, brushOverlay, tooltipValues) {
  featureAxis.each(function (d) {
    const processedDimensionName = cleanString(d.name);
    const [arrowTopHotspotX, arrowTopHotspotY] = getCursorHotspot(
      getArrowTopCursorMeta(),
      12,
    );
    const g = select(this)
      .append("g")
    ...
    g.append("use")
      ...
      .style(
        "cursor",
        `url('data:image/svg+xml,${encodeURIComponent(applyThemeToSvg(setSize(getArrowTopCursor(), 12)))}') ${arrowTopHotspotX} ${arrowTopHotspotY}, auto`,
      );
    const hit = g
      .append("rect")
...
function setBrushDown(featureAxis, brushOverlay, tooltipValues) {
  featureAxis.each(function (d) {
    const processedDimensionName = cleanString(d.name);
    const [arrowBottomHotspotX, arrowBottomHotspotY] = getCursorHotspot(
      getArrowBottomCursorMeta(),
      12,
    );
    const g = select(this)
      .append("g")
    ...
    g.append("use")
      ...
      .style(
        "cursor",
        `url('data:image/svg+xml,${encodeURIComponent(applyThemeToSvg(setSize(getArrowBottomCursor(), 12)))}') ${arrowBottomHotspotX} ${arrowBottomHotspotY}, auto`,
      );
    const hit = g
      .append("rect")
      .attr("class", "spcd3-handle-hitbox")
...
function brushDown(cleanDimensionName, event, d, tooltipValues, window) {
  const [arrowTopAndBottomHotspotX, arrowTopAndBottomHotspotY] =
    getCursorHotspot(getArrowTopAndBottomMeta(), 20);
  ...
  if (isAtTopHandle(yPosTop) && isAtBottom(yPosBottom)) {
    ...
    select("#rect_" + cleanDimensionName).style(
      "cursor",
      `url('data:image/svg+xml,${encodeURIComponent(applyThemeToSvg(setSize(getArrowTopAndBottom(), 20)))}') ${arrowTopAndBottomHotspotX} ${arrowTopAndBottomHotspotY}, auto`,
    );
  }
...
function handleRangeButton(dimension) {
  ...
  select("#buttonRange").on("click", () => {
    ...
    let isOk = true;
    if (inverted) {
      if (max < getMinValue(dimension) || min > getMaxValue(dimension)) {
        errorMessage.text(
      ...
      if (min > getMinValue(dimension) || max < getMaxValue(dimension)) {
        errorMessage.text(
...
function setCursorForDimensions(d, featureAxis) {
  if (getDimensionPosition(d.name) == 0) {
    ...
    featureAxis
      ...
      .style(
        "cursor",
        `url('data:image/svg+xml,${encodeURIComponent(applyThemeToSvg(setSize(getArrowRight(), 14)))}') ${hotspotX} ${hotspotY}, auto`,
      );
    ...
    featureAxis
      ...
      .style(
        "cursor",
        `url('data:image/svg+xml,${encodeURIComponent(applyThemeToSvg(setSize(getArrowLeft(), 14)))}') ${hotspotX} ${hotspotY}, auto`,
      );
  } else {
    const [hotspotX, hotspotY] = getCursorHotspot(
      getArrowLeftAndRightMeta(),
      14,
    );
    featureAxis
      .select(".dimension")
      .style(
        "cursor",
        `url('data:image/svg+xml,${encodeURIComponent(applyThemeToSvg(setSize(getArrowLeftAndRight(), 14)))}') ${hotspotX} ${hotspotY}, auto`,
      );
  }
}
...
function onDragEventHandler(featureAxis) {
  {
    return function onDrag(d) {
      ...
      parcoords.newFeatures.sort((a, b) => {
        return (
          position(b, parcoords.dragging, parcoords.xScales) -
...
function requireCjs$1() {
  ...
  (function (module, exports$1) {
    ...
    function element(matchRoot) {
      ...
      const excluded = matchRoot
        ? false
        : parsingState.options.filter(node) === false;
      // attributes
      while (!(eos() || is(">") || is("?>") || is("/>"))) {
      ...
      if (parsingState.options.strictMode) {
        ...
        if (parsingState.xml.startsWith(closingTag)) {
          ...
          throw new ParsingError(
            "Failed to parse XML",
            `Closing tag not matching "${closingTag}"`,
          );
        }
      } else {
        match(/^<\/[\w-:.\u00C0-\u00FF]+\s*>/);
...
function requireCjs() {
  ...
  (function (module, exports$1) {
    ...
    function isPathMatchingIgnoredPaths(path, ignoredPaths) {
      ...
      return (
        ignoredPaths.includes(pathLastPart) || ignoredPaths.includes(fullPath)
      );
    }
    function processElementNode(node, state, preserveSpace) {
      ...
      if (
        ...
        const selfClosingNodeClosingTag = state.options
        ...
        appendContent(state, ">");
        state.level++;
        let nodePreserveSpace =
          node.attributes["xml:space"] === "preserve" || preserveSpace;
        let ignoredPath = false;
        if (!nodePreserveSpace && state.options.ignoredPaths) {
          ignoredPath = isPathMatchingIgnoredPaths(
            state.path,
            state.options.ignoredPaths,
          );
        ...
        if (!nodePreserveSpace && state.options.collapseContent) {
          ...
          nodeChildren.forEach(function (child, index) {
            if (child.type === "Text") {
              if (child.content.includes("\n")) {
                ...
                (index === 0 || index === nodeChildren.length - 1) &&
                !preserveSpace
              ) {
          ...
          if (
            containsTextNodes &&
            (!containsNonTextNodes || !containsTextNodesWithLineBreaks)
          ) {
    ...
    function formatXml(xml, options = {}) {
      options.indentation =
        "indentation" in options ? options.indentation : "    ";
      options.collapseContent = options.collapseContent === true;
      options.lineSeparator =
        "lineSeparator" in options ? options.lineSeparator : "\r\n";
      options.whiteSpaceAtEndOfSelfclosingTag =
        options.whiteSpaceAtEndOfSelfclosingTag === true;
      options.throwOnFailure = options.throwOnFailure !== false;
      options.attributeQuotes =
        "attributeQuotes" in options ? options.attributeQuotes : "double";
      try {
...
function createSvgString() {
  ...
  const hiddenDims = getAllHiddenDimensionNames();
  let yScalesForDownload = setupYScales(
    parcoords.features,
    parcoords.newDataset,
  );
  let yAxisForDownload = setupYAxis(
    yScalesForDownload,
    parcoords.newDataset,
    hiddenDims,
  );
...
function alignToolbarWithLeftmostAxisLabels() {
  ...
  const axisNodes = Array.from(
    document.querySelectorAll("#spcd3-pc_svg .dimensions"),
  );
...
function setUpParcoordData(data, newFeatures) {
  ...
  for (let i = 0; i < newFeatures.length; i++) {
    ...
    let min;
    if (
      isNaN(Math.max(...parcoords.newDataset.map((o) => o[newFeatures[i]])))
    ) {
      const sorted = [
  ...
  invertIcon
    ...
    .each(function (d) {
      ...
      select(this)
        ...
        .style(
          "cursor",
          `url('data:image/svg+xml,${encodeURIComponent(applyThemeToSvg(setSize(getArrowDownCursor(), 12)))}') ${hotspotX} ${hotspotY}, auto`,
        );
    });
  invertIcon
    .append("svg")
